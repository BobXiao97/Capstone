...
def massing(site,building_list,iteration,stopping):
    ...
    while(success==0):
        ...
        for i in range(0,len(building_list)):
            ...
            angle=round(random.uniform(40,50),1)
    ...
    while(curr_iter<iteration and count_not_improve<stopping):
        ...
        z=0 #an indicator of whether the random design fits the constraints
        new_rectangle=[]
        x_shift=round(random.uniform(-site_x,site_x),1)
        y_shift=round(random.uniform(-site_y,site_y),1)
        for pos in position:
            pos[0]+=x_shift
            pos[1]+=y_shift
...
print(massing([200,200],[[50,30,'private'],[50,30,'private'],[50,30,'private'],[100,60,'public']],1000000,10000))     